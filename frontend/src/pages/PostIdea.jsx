...
const C = {
  ...
  border:  "rgba(48,54,61,0.9)", accent: "#58a6ff",
    ...
    <div style={{ position:"relative", width:"120px", height:"120px", flexShrink:0 }}>
      <svg width="120" height="120" viewBox="0 0 120 120">
        ...
        <circle cx="60" cy="60" r={r} fill="none" stroke={color} strokeWidth="10"
          ...
          strokeLinecap="round" style={{ transition:"stroke-dasharray 1.2s cubic-bezier(0.16,1,0.3,1)" }}
        />
      </svg>
...
function SuccessToast({ message, onDone }) {
  ...
  return (
    <div style={{ position:"fixed", top:"24px", left:"50%", transform:"translateX(-50%)", background:C.green, color:"#0d1117", padding:"12px 24px", borderRadius:"8px", fontWeight:"700", fontSize:"13px", zIndex:2000, boxShadow:"0 8px 32px rgba(0,0,0,0.4)", display:"flex", alignItems:"center", gap:"8px" }}>
      ✅ {message}
    </div>
  );
}
...
export default function PostIdea() {
  ...
  // ── FIX: replaced alert() + manual reset with navigate() ──
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.domain || !form.title) {
      setToast({ message: "Please fill in Title and Domain" });
      return;
    }
    setLoading(true);
    try {
      const token = localStorage.getItem("token");
      await axios.post("http://localhost:5000/api/innovator/upload-idea", form, {
        headers: { Authorization: `Bearer ${token}` },
      });

      // Show success toast then navigate away — no more alert()
      setToast({ message: "Idea submitted successfully 🚀" });
      setTimeout(() => navigate("/innovator/dashboard"), 2000);
      ...
      if (err.response?.data?.duplicate) {
        ...
        setToast({ message: err.response?.data?.message || "Idea cannot be uploaded" });
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:"'IBM Plex Sans',sans-serif", color:C.text }}>
      ...
      {/* Toast */}
      {toast && <SuccessToast message={toast.message} onDone={() => setToast(null)} />}
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"16px", marginBottom:"16px" }}>
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"16px" }}>
              <label style={{ fontSize:"10px", fontWeight:"700", color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em", display:"block", marginBottom:"8px" }}>
                Funding Received ($) <span style={{ color:C.dim, fontWeight:"400", textTransform:"none" }}>— optional</span>
              </label>
              <input
                type="text" inputMode="numeric" name="funding_total_usd" value={form.funding_total_usd}
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === '' || /^[0-9]+$/.test(val)) {
                    setForm({ ...form, funding_total_usd: val === '' ? '' : parseFloat(val) });
                  }
                }}
                onFocus={()=>setFocusedField("funding")} onBlur={()=>setFocusedField(null)}
                style={inputStyle("funding")}
              />
              <div style={{ fontSize:"9px", color:C.dim, marginTop:"5px" }}>Capital already raised — helps AI gauge runway risk</div>
            </div>

            {/* ── Funding Goal (Budget) ── */}
            <div style={{ background:C.surface, border:`1px solid rgba(88,166,255,.25)`, borderRadius:"8px", padding:"16px", position:"relative" }}>
              <label style={{ fontSize:"10px", fontWeight:"700", color:C.accent, textTransform:"uppercase", letterSpacing:"0.07em", display:"block", marginBottom:"8px" }}>
                🎯 Funding Goal (₹) <span style={{ color:C.dim, fontWeight:"400", textTransform:"none" }}>— optional</span>
              </label>
              <input
                type="text"
                inputMode="numeric"
                ...
                onChange={(e) => {
                  const val = e.target.value;
                 if (val === '' || /^[0-9]+$/.test(val)) {
                   setForm({ ...form, budget: val === '' ? '' : parseFloat(val) });
                 }
                }}                
                onFocus={()=>setFocusedField("budget")}
                onBlur={()=>setFocusedField(null)}
                style={{ ...inputStyle("budget"), border:`1px solid ${focusedField==="budget"?C.accent:"rgba(88,166,255,.25)"}` }}
                placeholder="e.g. 500000"
              />
              <div style={{ fontSize:"9px", color:C.dim, marginTop:"5px" }}>
                Set a target funding amount. When reached, you'll be prompted to raise the goal or close the listing.
              </div>
            </div>
            ...
            {/* ── Stage ── */}
            <div style={{ gridColumn:"1/-1", background:C.surface, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"16px" }}>
              <label style={{ fontSize:"10px", fontWeight:"700", color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em", display:"block", marginBottom:"12px" }}>Current Stage</label>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:"10px" }}>
                {STAGES.map(s => (
                  <button key={s.value} type="button" onClick={() => setForm({...form, stage:s.value})}
                    style={{ padding:"10px", borderRadius:"6px", cursor:"pointer", textAlign:"left", border:`1px solid ${form.stage===s.value?C.accent:C.border}`, background:form.stage===s.value?"rgba(88,166,255,.1)":C.surface2, transition:"all .15s", minHeight:"60px" }}>
            ...
            <div style={{ gridColumn:"1/-1", background:C.surface, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"16px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"8px" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
                  <span style={{ fontSize:"9px", color:form.problem.length>=100?C.green:form.problem.length>0?C.orange:C.dim, fontFamily:"'IBM Plex Mono',monospace" }}>{form.problem.length} chars</span>
              ...
              <textarea name="problem" value={form.problem} onChange={handleChange} rows={5}
                placeholder="What problem are you solving? Who faces it and how often? What does the current situation look like without your solution?"
                onFocus={()=>setFocusedField("problem")} onBlur={()=>setFocusedField(null)}
                style={inputStyle("problem")}
              />
              <div style={{ fontSize:"9px", color:C.dim, marginTop:"5px" }}>💡 Describe the pain point clearly. Investors want to see that you understand the problem deeply.</div>
            </div>
            ...
            <div style={{ gridColumn:"1/-1", background:C.surface, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"16px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"8px" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
                  <span style={{ fontSize:"9px", color:form.solution.length>=100?C.green:form.solution.length>0?C.orange:C.dim, fontFamily:"'IBM Plex Mono',monospace" }}>{form.solution.length} chars</span>
              ...
              <textarea name="solution" value={form.solution} onChange={handleChange} rows={5}
                placeholder="How does your idea solve the problem? What makes your solution different from existing alternatives?"
                onFocus={()=>setFocusedField("solution")} onBlur={()=>setFocusedField(null)}
                style={inputStyle("solution")}
              />
              <div style={{ fontSize:"9px", color:C.dim, marginTop:"5px" }}>💡 Highlight what makes your solution unique. Investors look for differentiation.</div>
            </div>
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"16px" }}>
              ...
              <textarea name="market" value={form.market} onChange={handleChange} rows={4}
                placeholder="e.g. SMEs in India, college students aged 18–25, rural farmers in Tamil Nadu..."
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"16px" }}>
              ...
              <textarea name="revenue" value={form.revenue} onChange={handleChange} rows={4}
                placeholder="e.g. Subscription ₹499/month, 2% commission per transaction, freemium with paid upgrades..."
          ...
          <div style={{ background:"rgba(88,166,255,.06)", border:"1px solid rgba(88,166,255,.2)", borderRadius:"7px", padding:"12px 16px", marginBottom:"14px", display:"flex", gap:"10px", alignItems:"flex-start" }}>
            ...
            <div>
              ...
              <div style={{ fontSize:"10px", color:C.muted, lineHeight:"1.7" }}>
                Problem & solution 100+ chars · Clear revenue model · Specific target market · Stage set to Prototype or higher · Funding goal set
              </div>
            </div>
          </div>

          <button type="submit" disabled={loading} style={{ width:"100%", padding:"14px", background:loading?C.surface2:C.accent, color:loading?C.muted:"#0d1117", border:"none", borderRadius:"7px", fontWeight:"700", fontSize:"13px", cursor:loading?"not-allowed":"pointer", fontFamily:"'IBM Plex Sans',sans-serif", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px" }}>
            {loading ? (<><div style={{ width:"14px", height:"14px", border:`2px solid ${C.dim}`, borderTopColor:C.accent, borderRadius:"50%", animation:"spin .8s linear infinite" }}/>Submitting...</>) : "🚀 Submit Idea for AI Analysis"}
          </button>
        </form>
      </div>
      ...
      {/* ── Similarity Modal ── */}
      {similarityData && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"1rem" }} onClick={()=>setSimilarityData(null)}>
          <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:"10px", width:"100%", maxWidth:"580px", maxHeight:"90vh", overflowY:"auto", boxShadow:"0 32px 80px rgba(0,0,0,0.5)" }} onClick={e=>e.stopPropagation()}>
            <div style={{ background:C.surface2, borderBottom:`1px solid ${C.border}`, padding:"14px 18px", borderRadius:"10px 10px 0 0", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:"9px", color:C.muted }}>AI-powered similarity analysis</div>
                </div>
              </div>
            ...
            <div style={{ padding:"16px 18px 20px" }}>
              ...
              <div style={{ background:`${verdictColor}10`, border:`1px solid ${verdictColor}40`, color:verdictColor, borderRadius:"6px", padding:"8px 14px", fontSize:"12px", fontWeight:"700", textAlign:"center", marginBottom:"14px" }}>{verdictText}</div>
              ...
              <div style={{ background:"rgba(88,166,255,.06)", border:"1px solid rgba(88,166,255,.2)", borderRadius:"7px", padding:"12px 14px", marginBottom:"14px" }}>
                ...
                <div style={{ fontSize:"11px", color:C.muted, lineHeight:"1.7" }}>Focus on a <span style={{ color:C.text, fontWeight:"600" }}>unique target segment</span>, a novel technical approach, or a geography/use-case the existing idea doesn't cover.</div>
              </div>
