...
const STRATEGIC_ADVICE = {
  ...
  "Underfunded — Seek Early Capital":  { color:C.orange, bg:"rgba(240,136,62,.08)", border:"rgba(240,136,62,.2)", icon:"💸", advice:"Seek early capital to extend runway. Even ₹83L+ removes critical penalty." },
  "High Risk — Needs Validation":      { color:C.red,    bg:"rgba(248,81,73,.08)",  border:"rgba(248,81,73,.2)",  icon:"🔴", advice:"Multiple risk signals. Focus on milestones and funding to improve score." },
  "Moderate Potential — Watch for Traction":{ color:C.orange,bg:"rgba(240,136,62,.08)",border:"rgba(240,136,62,.2)",icon:"📈",advice:"Promising concept — needs stronger traction signals to attract investors." },
  ...
  "Proven Traction — Investment Ready":{ color:C.green,  bg:"rgba(63,185,80,.08)",  border:"rgba(63,185,80,.2)",  icon:"✅", advice:"Strong traction with live product and milestones. Ready for investor conversations." },
};

/* ── HOW TO IMPROVE SCORE tips ── */
const IMPROVEMENT_TIPS = [
  { icon:"💰", title:"Raise early funding",       detail:"Getting even ₹42–83L removes the critical underfunding penalty from your score.", impact:"+5–10%" },
  { icon:"🏆", title:"Add 3+ milestones",          detail:"Hitting milestones proves execution. 3+ unlocks a scoring bonus for demonstrated progress.", impact:"+4%" },
  { icon:"🤝", title:"Grow your network",          detail:"5+ connections removes the weak network flag. Advisors, mentors, early users all count.", impact:"+2–4%" },
  { icon:"⚡", title:"Move to Prototype or MVP",   detail:"Idea-stage carries an 8% penalty. Even a basic prototype removes it immediately.", impact:"+8%" },
  { icon:"🌐", title:"Build a web product",        detail:"Digital products get a scalability bonus. Mention your web presence if applicable.", impact:"+2%" },
  { icon:"📊", title:"Complete multiple funding rounds", detail:"Even small seed rounds signal investor confidence and unlock the funding rounds bonus.", impact:"+4%" },
];
  ...
  return (
    <div style={{ marginBottom:"8px" }}>
      <div style={{ display:"flex",justifyContent:"space-between",marginBottom:"3px" }}>
        <span style={{ fontSize:"10px",color:C.text,fontWeight:"600" }}>{label}</span>
        <span style={{ fontSize:"10px",fontWeight:"700",color,fontFamily:"'IBM Plex Mono',monospace" }}>{score}%</span>
      </div>
      <div style={{ height:"4px",background:C.surface,borderRadius:"3px",overflow:"hidden" }}>
        <div style={{ height:"100%",width:`${w}%`,background:color,borderRadius:"3px",transition:"width .9s ease" }}/>
      </div>
    </div>
  );
}
...
/* ── Input style helper ── */
const inputStyle = (focused, name, focusedField) => ({
    ...
    <div style={{ background:C.bg,minHeight:"100vh",fontFamily:"'IBM Plex Sans',sans-serif",color:C.text }}>
      <div style={{ maxWidth:"1000px",margin:"0 auto",padding:"20px 24px" }}>
        ...
        <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:"8px",padding:"14px 18px",marginBottom:"16px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div>
            ...
            <div style={{ fontSize:"10px",color:C.muted,marginTop:"2px" }}>{ideas.length} submission{ideas.length!==1?"s":""} · click View Analysis to see your AI score</div>
          </div>
        ...
        {ideas.length === 0 ? (
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:"10px" }}>
            {ideas.map((idea)=>(
              <div key={idea._id} style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:"8px",padding:"16px 18px",transition:"border-color .15s" }}
                ...
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px",gap:"10px",flexWrap:"wrap" }}>
                  ...
                  <div style={{ display:"flex",gap:"6px" }}>
                    <button onClick={()=>handleEditClick(idea)} style={{ padding:"6px 12px",background:"transparent",color:C.muted,border:`1px solid ${C.border}`,borderRadius:"5px",fontSize:"11px",cursor:"pointer",fontFamily:"'IBM Plex Sans',sans-serif",transition:"all .15s" }}
                      onMouseEnter={e=>{e.currentTarget.style.color=C.text;e.currentTarget.style.borderColor=C.muted;}}
                      onMouseLeave={e=>{e.currentTarget.style.color=C.muted;e.currentTarget.style.borderColor=C.border;}}>
                      ✏️ Edit
                    </button>
                    <button onClick={()=>handleViewAnalysis(idea)} style={{ padding:"6px 14px",background:C.accent,color:"#0d1117",border:"none",borderRadius:"5px",fontSize:"11px",fontWeight:"700",cursor:"pointer",fontFamily:"'IBM Plex Sans',sans-serif" }}>
                      🤖 View Analysis
                    </button>
                  </div>
                </div>
            ...
            <form onSubmit={handleUpdate} style={{ padding:"16px 18px 20px",display:"flex",flexDirection:"column",gap:"12px" }}>
              ...
              <div>
                ...
                <input name="title" value={editForm.title} onChange={handleEditChange} required
                  ...
                  style={inputStyle(true,"e_title",focusedField)}/>
              </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px" }}>
                <div>
                  ...
                  <select name="domain" value={editForm.domain} onChange={handleEditChange} required
                    ...
                    style={inputStyle(true,"e_domain",focusedField)}>
                ...
                <div>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px" }}>
                    {STAGES.map(s=>(
                      <button key={s} type="button" onClick={()=>setEditForm({...editForm,stage:s})} style={{ padding:"5px 8px",borderRadius:"5px",cursor:"pointer",fontSize:"10px",fontWeight:"600",fontFamily:"'IBM Plex Sans',sans-serif",border:`1px solid ${editForm.stage===s?(STAGE_COLORS[s]||C.accent):C.border}`,background:editForm.stage===s?`${STAGE_COLORS[s]||C.accent}15`:"transparent",color:editForm.stage===s?(STAGE_COLORS[s]||C.accent):C.muted,transition:"all .15s" }}>
                        {s}
                      </button>
                    ))}
                  </div>
              ...
              <div>
                ...
                <textarea name="problem" value={editForm.problem} rows={4} onChange={handleEditChange}
                  ...
                  style={inputStyle(true,"e_problem",focusedField)}/>
              </div>
              ...
              <div>
                ...
                <textarea name="solution" value={editForm.solution} rows={4} onChange={handleEditChange}
                  ...
                  style={inputStyle(true,"e_solution",focusedField)}/>
              </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px" }}>
                <div>
                  ...
                  <textarea name="market" value={editForm.market} rows={3} onChange={handleEditChange}
                    ...
                    style={inputStyle(true,"e_market",focusedField)}/>
                </div>
                <div>
                  ...
                  <textarea name="revenue" value={editForm.revenue} rows={3} onChange={handleEditChange}
                    ...
                    style={inputStyle(true,"e_revenue",focusedField)}/>
                </div>
              </div>

              <div style={{ display:"flex",gap:"8px",paddingTop:"4px" }}>
                <button type="submit" style={{ flex:1,padding:"10px",background:C.accent,color:"#0d1117",border:"none",borderRadius:"6px",fontWeight:"700",fontSize:"12px",cursor:"pointer",fontFamily:"'IBM Plex Sans',sans-serif" }}>
                  Save Changes
                </button>
                <button type="button" onClick={()=>{setEditIdea(null);setEditForm(null);}} style={{ padding:"10px 18px",background:"transparent",color:C.muted,border:`1px solid ${C.border}`,borderRadius:"6px",fontSize:"12px",cursor:"pointer",fontFamily:"'IBM Plex Sans',sans-serif" }}>
                  Cancel
                </button>
              </div>
            </form>
      ...
      {analysisIdea && (
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(4px)",display:"flex",alignItems:isFullscreen?"flex-start":"center",justifyContent:"center",zIndex:1000,padding:isFullscreen?"0":"1rem" }}
          ...
          <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:isFullscreen?"0":"10px",width:"100%",maxWidth:isFullscreen?"100%":"760px",height:isFullscreen?"100vh":"auto",maxHeight:isFullscreen?"100vh":"92vh",overflowY:"auto",boxShadow:"0 32px 80px rgba(0,0,0,0.6)",display:"flex",flexDirection:"column",transition:"all .2s ease" }}
            onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ background:C.surface2,borderBottom:`1px solid ${C.border}`,padding:"13px 18px",borderRadius:isFullscreen?"0":"10px 10px 0 0",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,position:isFullscreen?"sticky":"static",top:0,zIndex:10 }}>
              ...
              <div style={{ display:"flex",gap:"6px",alignItems:"center" }}>
                ...
                <button
                  ...
                  style={{ background:"rgba(255,255,255,.06)",border:`1px solid ${C.border}`,color:C.muted,width:"26px",height:"26px",borderRadius:"5px",cursor:"pointer",fontSize:"12px",display:"flex",alignItems:"center",justifyContent:"center",transition:"background .15s" }}
                ...
                <div style={{ flex:1,display:"flex",flexDirection:"column" }}>
                  ...
                  <div style={{ borderBottom:`1px solid ${C.border}`,display:"flex",padding:"0 18px",flexShrink:0 }}>
                    {ANALYSIS_TABS.map(({k,l})=>(
                      <button key={k} onClick={()=>setActiveTab(k)} style={{ padding:"0 14px",height:"38px",border:"none",borderBottom:activeTab===k?`2px solid ${C.accent}`:"2px solid transparent",background:"transparent",color:activeTab===k?C.accent:C.muted,fontWeight:activeTab===k?"600":"400",fontSize:"11px",cursor:"pointer",transition:"all .15s",fontFamily:"'IBM Plex Sans',sans-serif" }}>{l}</button>
                    ))}
                  </div>

                  <div style={{ padding:"16px 18px 20px",flex:1,overflow:"auto",animation:"fadeUp .25s ease" }}>

                    {/* ── TAB: SCORE & FORECAST ── */}
                    {activeTab==="score" && (
                      <div>
                        {/* Score breakdown + strategic + confidence */}
                        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px",marginBottom:"14px" }}>
                          ...
                          <div style={{ background:C.surface2,border:`1px solid ${C.border}`,borderRadius:"8px",padding:"14px",display:"flex",flexDirection:"column",alignItems:"center",gap:"12px" }}>
                            ...
                            <div style={{ width:"100%" }}>
                              {[
                                { label:"Initial Score",    val:rawScore,  color:C.accent   },
                                { label:"Calibrated Score", val:score,     color:scoreColor },
                              ].map((s,i)=>(
                          ...
                          <div style={{ background:C.surface2,border:`1px solid ${C.border}`,borderRadius:"8px",padding:"14px" }}>
                            ...
                            <div style={{ fontSize:"9px",color:C.dim,marginTop:"10px",lineHeight:"1.5" }}>Projected growth trajectory based on sector benchmarks and founding year</div>
                          </div>
                        </div>
                        ...
                        {analysisData.model_warnings?.length>0 && (
                          <div style={{ background:"rgba(210,153,34,.06)",border:"1px solid rgba(210,153,34,.2)",borderRadius:"7px",padding:"12px 14px",marginBottom:"14px" }}>
                            <div style={{ fontSize:"9px",fontWeight:"700",color:C.yellow,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:"8px" }}>⚠️ Why your score is {score}% — risk flags detected</div>
                    ...
                    {activeTab==="factors" && (
                      <div>
                        ...
                        {shapAllZero ? (
                          <div>
                            <div style={{ fontSize:"9px",color:C.yellow,padding:"8px 10px",background:"rgba(210,153,34,.08)",border:"1px solid rgba(210,153,34,.2)",borderRadius:"5px",marginBottom:"12px" }}>
                              ⚠️ Detailed factor weights unavailable — showing signal summary instead.
                            </div>
                            {[...pros.map(p=>({name:p.name,icon:p.icon,positive:true})),...cons.map(c=>({name:c.name,icon:c.icon,positive:false}))].map((d,i,arr)=>(
                              <div key={i} style={{ display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px" }}>
                                <div style={{ width:"80px",fontSize:"10px",color:C.text,fontWeight:"600",textAlign:"right",flexShrink:0 }}>{d.icon} {d.name}</div>
                                <div style={{ flex:1,height:"8px",background:C.surface,borderRadius:"4px",overflow:"hidden" }}>
                                  <div style={{ width:`${Math.round(100-i*(100/arr.length))}%`,height:"100%",background:d.positive?C.green:C.red,borderRadius:"4px" }}/>
                                </div>
                                <div style={{ fontSize:"9px",padding:"1px 6px",borderRadius:"3px",background:d.positive?"rgba(63,185,80,.12)":"rgba(248,81,73,.12)",color:d.positive?C.green:C.red,flexShrink:0 }}>{d.positive?"↑":"↓"}</div>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div style={{ display:"flex",flexDirection:"column",gap:"8px" }}>
                            {shapData.map((d,i)=>(
                              <div key={i} style={{ display:"flex",alignItems:"center",gap:"8px" }}>
                                ...
                                <div style={{ width:"40px",fontSize:"9px",fontWeight:"700",fontFamily:"'IBM Plex Mono',monospace",color:d.positive?C.green:C.red,flexShrink:0 }}>{d.label}</div>
                        ...
                        <div style={{ fontSize:"8px",color:C.dim,marginTop:"12px",borderTop:`1px solid ${C.border}`,paddingTop:"8px" }}>
                          Larger bar = stronger influence on your score. Green = helping your score · Red = hurting your score.
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab==="improve" && (
                      <div>
                        <SectionHeader title="How to Improve Your Score" sub="Specific actions based on how the scoring engine works — ordered by impact"/>
                        ...
                        <div style={{ background:`rgba(88,166,255,.06)`,border:`1px solid rgba(88,166,255,.2)`,borderRadius:"7px",padding:"12px 14px" }}>
                          <div style={{ fontSize:"9px",fontWeight:"700",color:C.accent,marginBottom:"6px",textTransform:"uppercase",letterSpacing:"0.06em" }}>💡 Current score: {score}% · Target: 70%+</div>
                          <div style={{ fontSize:"10px",color:C.muted,lineHeight:"1.7" }}>
                            Focus on the highest-impact items first. Moving from Idea to Prototype alone adds up to 8%. Combined with adding milestones and growing your network, you can realistically reach 50–70%+ from almost any starting point.
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ fontSize:"9px",color:C.dim,marginTop:"14px",padding:"8px 12px",background:C.surface2,borderRadius:"5px",lineHeight:"1.6" }}>
                      ℹ️ Innovest's scoring engine analyses your venture across multiple dimensions — funding, network, traction, market fit, and domain context. This is a decision-support signal, not a financial guarantee.
