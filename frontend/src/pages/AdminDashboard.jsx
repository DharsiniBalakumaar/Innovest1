...
function normaliseDomain(arr = []) {
  return arr
    .map(item => ({ domain: item.domain || item._id || "Other", count: item.count || 0 }))
    .filter(item => item.domain);
}

/* KEY FIX: strip any leading "uploads/" or "/" so the URL never doubles */
function cleanFilename(raw = "") {
  return raw.replace(/^uploads[\\/]/, "").replace(/^\/+/, "");
}

function Spinner() {
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"48px", gap:"14px" }}>
      <div style={{ width:"28px", height:"28px", border:`2px solid ${C.dim}`, borderTopColor:C.accent, borderRadius:"50%", animation:"spin .8s linear infinite" }}/>
      <span style={{ fontSize:"12px", color:C.muted, fontFamily:"'DM Sans',sans-serif", letterSpacing:"0.05em" }}>Loading data...</span>
    </div>
  );
}
...
function Modal({ title, onClose, maxWidth="860px", children }) {
  ...
  return (
    <div
      style={{ position:"fixed", inset:0, background:"rgba(4,8,20,0.92)", backdropFilter:"blur(8px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"16px" }}
      onClick={onClose}
    >
      <div
        style={{ background:C.s1, border:`1px solid ${C.border2}`, borderRadius:max?"0":"16px", width:max?"100vw":"100%", maxWidth:max?"100vw":maxWidth, height:max?"100vh":"auto", maxHeight:max?"100vh":"90vh", display:"flex", flexDirection:"column", boxShadow:"0 40px 100px rgba(0,0,0,.9)", transition:"all .2s" }}
      ...
      >
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 20px", borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
          <span style={{ fontSize:"14px", fontWeight:"700", color:C.text, fontFamily:"'Syne',sans-serif" }}>{title}</span>
          <div style={{ display:"flex", gap:"6px" }}>
            <button onClick={()=>setMax(p=>!p)} style={{ background:C.s3, border:`1px solid ${C.border}`, borderRadius:"6px", width:"28px", height:"28px", color:C.muted, cursor:"pointer", fontSize:"13px" }}>{max?"⊡":"⛶"}</button>
            <button onClick={onClose} style={{ background:C.s3, border:`1px solid ${C.border}`, borderRadius:"6px", width:"28px", height:"28px", color:C.muted, cursor:"pointer", fontSize:"13px" }}>✕</button>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"10px" }}>
        <span style={{ fontSize:"10px", fontWeight:"700", color:C.muted, textTransform:"uppercase", letterSpacing:"0.1em", fontFamily:"'DM Sans',sans-serif" }}>{label}</span>
        <span style={{ fontSize:"17px" }}>{icon}</span>
      </div>
      <div style={{ fontSize:"28px", fontWeight:"800", color, fontFamily:"'DM Mono',monospace", lineHeight:1, marginBottom:"6px" }}>{value}</div>
...
function SectionTitle({ title, sub }) {
  return (
    <div style={{ marginBottom:"16px" }}>
      ...
      {sub && <div style={{ fontSize:"11px", color:C.muted, marginTop:"3px", fontFamily:"'DM Sans',sans-serif" }}>{sub}</div>}
    </div>
  );
}

function ChartCard({ title, sub, children, minH=260, style:sx={} }) {
  return (
    <div style={{ background:C.s2, border:`1px solid ${C.border}`, borderRadius:"12px", padding:"18px", animation:"fadeUp .5s ease-out", minHeight:`${minH}px`, ...sx }}>
...
const TT = ({ contentStyle, itemStyle, labelStyle, ...props }) => (
  <Tooltip
    ...
    labelStyle={{ color:C.muted, marginBottom:"4px", ...labelStyle }}
    {...props}
  />
);
...
function EmptyChart({ message = "No data available yet." }) {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"180px", color:C.muted, fontSize:"13px", fontFamily:"'DM Sans',sans-serif", flexDirection:"column", gap:"8px" }}>
...
/* Professional ghost-style action buttons */
function ActionButton({ onClick, label, variant }) {
  const [hov, setHov] = useState(false);
  const cfg = {
    approve: { base: { border:`1.5px solid ${C.green}45`, color:C.green, background:`${C.green}0a` }, hover: { border:`1.5px solid ${C.green}`, color:C.green, background:`${C.green}18` } },
    reject:  { base: { border:`1.5px solid ${C.red}45`,   color:C.red,   background:`${C.red}0a`  }, hover: { border:`1.5px solid ${C.red}`,   color:C.red,   background:`${C.red}18`  } },
  };
  const s = cfg[variant];
  return (
    <button
      ...
      onMouseEnter={()=>setHov(true)}
      onMouseLeave={()=>setHov(false)}
      style={{ flex:1, padding:"12px 0", ...(hov?s.hover:s.base), borderRadius:"8px", cursor:"pointer", fontSize:"12px", fontWeight:"700", fontFamily:"'DM Sans',sans-serif", letterSpacing:"0.05em", textTransform:"uppercase", display:"flex", alignItems:"center", justifyContent:"center", gap:"7px", transition:"all .18s" }}
    >
      {label}
    </button>
  );
...
export default function AdminDashboard() {
  ...
  const handleViewIdea = async (idea) => {
    ...
    try {
      ...
      const aiInput = {
        age_first_funding_year: 1.0, age_last_funding_year: 2.0, relationships: 5,
        funding_rounds: idea.stage === "Live" ? 2 : 1,
        funding_total_usd: fundingMap[idea.stage] || 50000,
        milestones: milestoneMap[idea.stage] || 1, is_CA: 0,
        is_web: ["AI","Edtech","Fintech","Healthcare"].includes(idea.domain) ? 1 : 0,
        founded_year: new Date().getFullYear(),
        stage: idea.stage || "Idea", domain: idea.domain || "General",
      };
  ...
  const getReasons = (explanation) => {
    ...
    const pros = [], cons = [];
    Object.entries(explanation).forEach(([k, v]) => {
      const info = FEATURE_INFO[k];
      if (!info) return;
      if (v >= 0) pros.push({ ...info, k }); else cons.push({ ...info, k });
    });
    return { pros, cons };
  };
  ...
  const domainData   = normaliseDomain(stats.ideasByDomain || []);
  ...
  const domainPie = domainData.map((d, i) => ({ ...d, fill: PALETTE[i % PALETTE.length] }));
  ...
  const listMap = { pendingInnovators: pendingUsers, approvedInnovators: approvedUsers, pendingInvestors, approvedInvestors };
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", color:C.text, fontFamily:"'DM Sans',sans-serif", fontSize:"14px" }}>
      <div style={{ maxWidth:"1700px", margin:"0 auto", padding:"20px 24px" }}>
        ...
        {/* Header */}
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"20px", padding:"16px 22px", background:C.s2, border:`1px solid ${C.border}`, borderRadius:"12px" }}>
          <div>
            <div style={{ fontSize:"19px", fontWeight:"800", color:C.text, fontFamily:"'Syne',sans-serif", letterSpacing:"-0.02em" }}>🛡️ Admin Command Center</div>
          ...
          <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"6px", background:C.s3, border:`1px solid ${C.green}30`, borderRadius:"6px", padding:"6px 12px" }}>
              <span style={{ width:"6px", height:"6px", background:C.green, borderRadius:"50%", animation:"pulse 2s infinite", display:"inline-block" }}/>
        ...
        <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"20px", background:C.s2, padding:"8px", borderRadius:"10px", border:`1px solid ${C.border}` }}>
          {TABS.map(({ key, label, count }) => (
            <button key={key} onClick={()=>setActiveSection(key)} style={{
              ...
              {label}
              {count !== undefined && (
                <span style={{ background:activeSection===key?"rgba(0,0,0,.2)":"rgba(255,255,255,.08)", borderRadius:"4px", padding:"1px 6px", fontSize:"10px" }}>
                  {count}
                </span>
              )}
            </button>
        ...
        {activeSection === "overview" && (
          <div style={{ animation:"fadeUp .4s ease-out" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"2fr 1.2fr 1fr", gap:"12px", marginBottom:"12px" }}>
              <ChartCard title="Ideas by Domain" sub="Distribution across all startup categories" minH={280}>
                {domainData.length === 0 ? <EmptyChart message="No domain data yet."/> : (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={domainData} margin={{top:4,right:4,left:-20,bottom:0}}>
                      <CartesianGrid strokeDasharray="3 3" stroke={C.border} vertical={false}/>
                      <XAxis dataKey="domain" tick={{fill:C.muted,fontSize:11}} axisLine={false} tickLine={false}/>
                      ...
                      <Bar dataKey="count" name="Ideas" radius={[4,4,0,0]} maxBarSize={36}>
                        {domainData.map((_,i)=><Cell key={i} fill={PALETTE[i%PALETTE.length]}/>)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                )}
              ...
              <ChartCard title="User Distribution" sub="Role & approval status" minH={280}>
                {userDistData.length === 0 ? <EmptyChart message="No users registered yet."/> : (
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
              ...
              <ChartCard title="Platform Health" sub="Multi-dimension score" minH={280}>
                <ResponsiveContainer width="100%" height={220}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
              <ChartCard title="User Funnel" sub="Registered → Active" minH={260}>
              ...
              <ChartCard title="Domain Share" sub="Ideas per sector" minH={260}>
                {domainPie.length === 0 ? <EmptyChart message="No domain data yet."/> : (
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
              <ChartCard title="Platform Summary" sub="Live counts from database" minH={200}>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={i} style={{ borderBottom:`1px solid ${C.border}` }}>
                        <td style={{ padding:"8px 0", fontSize:"12px", color:C.muted }}>{row.label}</td>
              ...
              <ChartCard title="⚡ Pending Actions" sub="Require immediate review" minH={200}>
                {pendingTotal === 0 ? (
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:"8px", maxHeight:"280px", overflowY:"auto" }}>
                    {[
                      ...pendingUsers.slice(0,4).map(u=>({...u,_type:"Innovator"})),
                      ...pendingInvestors.slice(0,4).map(u=>({...u,_type:"Investor"})),
                    ].map(u=>(
                      <div key={u._id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 14px", background:C.s3, borderRadius:"8px", border:`1px solid ${C.border}` }}>
                        ...
                        <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
                          ...
                          <button onClick={()=>setSelectedUser(u)} style={{ background:`${C.accent}15`, border:`1px solid ${C.accent}40`, borderRadius:"6px", padding:"5px 12px", color:C.accent, cursor:"pointer", fontSize:"11px", fontWeight:"700", letterSpacing:"0.03em" }}>
                            Review
                          </button>
                        </div>
                      </div>
                    ...
                    {pendingTotal > 8 && (
                      <div style={{ fontSize:"11px", color:C.muted, textAlign:"center", paddingTop:"4px" }}>+{pendingTotal-8} more pending</div>
                    )}
                  </div>
                )}
              </ChartCard>
            ...
            <ChartCard title="Recent Idea Submissions" sub="Latest ventures on the platform" minH={100}>
              {ideas.length === 0 ? <EmptyChart message="No ideas submitted yet."/> : (
                <>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:"10px" }}>
                    {ideas.slice(0,6).map(idea=>(
                      <div key={idea._id} style={{ background:C.s3, border:`1px solid ${C.border}`, borderRadius:"8px", padding:"12px" }}>
                        ...
                        <button onClick={()=>handleViewIdea(idea)} style={{ width:"100%", padding:"6px", background:"transparent", border:`1px solid ${C.accent}40`, borderRadius:"5px", color:C.accent, cursor:"pointer", fontSize:"11px", fontWeight:"700" }}>
                          🤖 AI Analysis
                        </button>
                      </div>
                    ))}
        ...
        {["pendingInnovators","approvedInnovators","pendingInvestors","approvedInvestors"].includes(activeSection) && (
          <div style={{ animation:"fadeUp .4s ease-out" }}>
            <div style={{ background:C.s2, border:`1px solid ${C.border}`, borderRadius:"12px", padding:"18px" }}>
              <SectionTitle
                title={TABS.find(t=>t.key===activeSection)?.label?.replace(/[^\x20-\x7E]/g,"").trim() || activeSection}
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                  {(listMap[activeSection]||[]).map(u=>(
                    <div key={u._id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 18px", background:C.s3, borderRadius:"10px", border:`1px solid ${C.border}` }}>
                      ...
                      <button onClick={()=>setSelectedUser(u)} style={{ background:`${C.accent}12`, border:`1px solid ${C.accent}40`, borderRadius:"8px", padding:"9px 18px", color:C.accent, cursor:"pointer", fontSize:"12px", fontWeight:"700", letterSpacing:"0.03em" }}>
                        View KYC
                      </button>
                    </div>
                  ))}
          ...
          <div style={{ animation:"fadeUp .4s ease-out" }}>
            <div style={{ background:C.s2, border:`1px solid ${C.border}`, borderRadius:"12px", padding:"18px" }}>
              ...
              {ideas.length === 0 ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                  {ideas.map(idea=>(
                    <div key={idea._id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 18px", background:C.s3, borderRadius:"10px", border:`1px solid ${C.border}` }}>
                      <div style={{ flex:1 }}>
                      ...
                      <button onClick={()=>handleViewIdea(idea)} style={{ background:`${C.purple}15`, border:`1px solid ${C.purple}40`, borderRadius:"8px", padding:"9px 18px", color:C.purple, cursor:"pointer", fontSize:"12px", fontWeight:"700", marginLeft:"12px", flexShrink:0, letterSpacing:"0.03em" }}>
                        🤖 AI Analysis
                      </button>
                    </div>
                  ))}
      ...
      {/* ══════════════════════════════
          KYC MODAL — professional
      ══════════════════════════════ */}
      {selectedUser && (() => {
        const filename  = cleanFilename(selectedUser.documents?.identityProof || "");
        const docUrl    = filename ? `http://localhost:5000/uploads/${filename}` : null;
        const initials  = selectedUser.name
          ? selectedUser.name.trim().split(/\s+/).map(w=>w[0]).join("").toUpperCase().slice(0,2)
          : "?";
        const isPending = selectedUser.status === "pending";

        return (
          <Modal title="" onClose={()=>setSelectedUser(null)} maxWidth="560px">

            {/* Profile header */}
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"24px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"14px" }}>
                <div style={{ width:"52px", height:"52px", borderRadius:"50%", background:`${C.accent}18`, border:`1.5px solid ${C.accent}35`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"17px", fontWeight:"800", color:C.accent, fontFamily:"'DM Mono',monospace", flexShrink:0 }}>
                  {initials}
                </div>
                <div>
                  <div style={{ fontSize:"20px", fontWeight:"800", color:C.text, fontFamily:"'Syne',sans-serif", letterSpacing:"-0.02em", lineHeight:1.15 }}>
                    {selectedUser.name}
                  </div>
                  <div style={{ fontSize:"10px", color:C.muted, marginTop:"5px", textTransform:"uppercase", letterSpacing:"0.12em", fontFamily:"'DM Sans',sans-serif" }}>
                    KYC Review — {selectedUser.role}
                  </div>
                </div>
              </div>
              <div style={{ fontSize:"10px", fontWeight:"800", padding:"5px 14px", borderRadius:"20px", background:isPending?`${C.yellow}15`:`${C.green}15`, color:isPending?C.yellow:C.green, border:`1px solid ${isPending?C.yellow:C.green}40`, textTransform:"uppercase", letterSpacing:"0.1em", fontFamily:"'DM Mono',monospace" }}>
                {selectedUser.status}
              </div>
            </div>

            {/* Info grid */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1px", background:C.border, border:`1px solid ${C.border}`, borderRadius:"10px", overflow:"hidden", marginBottom:"16px" }}>
              {[
                { label:"Full Name",      value: selectedUser.name,                       hl: false },
                { label:"Email Address",  value: selectedUser.email,                      hl: true  },
                { label:"Phone Number",   value: selectedUser.phone    || "Not provided",  hl: false },
                { label:"Account Role",   value: selectedUser.role,                       hl: false },
                { label:"Account Status", value: selectedUser.status,                     hl: false },
                { label:"LinkedIn",       value: selectedUser.linkedin || "Not provided",  hl: true  },
              ].map((row, i) => (
                <div key={i} style={{ background:C.s3, padding:"13px 16px" }}>
                  <div style={{ fontSize:"9px", fontWeight:"800", color:C.muted, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:"5px", fontFamily:"'DM Sans',sans-serif" }}>{row.label}</div>
                  <div style={{ fontSize:"13px", fontWeight:"600", color:row.hl?C.accent:C.text, fontFamily:"'DM Sans',sans-serif", wordBreak:"break-all", lineHeight:1.4 }}>{row.value}</div>
                </div>
              ))}
            </div>

            {/* Identity document */}
            <div style={{ background:C.s3, border:`1px solid ${C.border2}`, borderRadius:"10px", padding:"16px", marginBottom:"16px" }}>
              <div style={{ fontSize:"9px", fontWeight:"800", color:C.muted, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:"12px", fontFamily:"'DM Sans',sans-serif" }}>
                Identity Document
              </div>
              {docUrl ? (
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:"12px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"12px", minWidth:0 }}>
                    <div style={{ width:"38px", height:"38px", background:`${C.accent}12`, border:`1px solid ${C.accent}25`, borderRadius:"8px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"16px", flexShrink:0 }}>📄</div>
                    <div style={{ minWidth:0 }}>
                      <div style={{ fontSize:"12px", fontWeight:"600", color:C.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:"260px" }}>{filename}</div>
                      <div style={{ fontSize:"10px", color:C.muted, marginTop:"3px" }}>Click "Open file" to review this document</div>
                    </div>
                  </div>
                  <a
                    href={docUrl}
                    target="_blank"
                    rel="noreferrer"
                    style={{ flexShrink:0, background:`${C.accent}12`, border:`1px solid ${C.accent}40`, borderRadius:"7px", padding:"8px 16px", color:C.accent, fontSize:"12px", fontWeight:"700", textDecoration:"none", whiteSpace:"nowrap", letterSpacing:"0.03em" }}
                  >
                    Open file ↗
                  </a>
                </div>
              ) : (
                <div style={{ display:"flex", alignItems:"center", gap:"10px", padding:"10px 14px", background:`${C.red}0a`, border:`1px solid ${C.red}22`, borderRadius:"8px" }}>
                  <span style={{ fontSize:"16px" }}>⚠️</span>
                  <div>
                    <div style={{ fontSize:"12px", fontWeight:"700", color:C.red }}>No document uploaded</div>
                    <div style={{ fontSize:"11px", color:C.muted, marginTop:"2px" }}>This user has not submitted an identity document.</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {isPending ? (
              <>
                <div style={{ display:"flex", alignItems:"flex-start", gap:"9px", marginBottom:"14px", padding:"10px 14px", background:`${C.yellow}08`, border:`1px solid ${C.yellow}1f`, borderRadius:"8px" }}>
                  <span style={{ fontSize:"13px", flexShrink:0 }}>ℹ️</span>
                  <span style={{ fontSize:"11px", color:C.muted, lineHeight:"1.6", fontFamily:"'DM Sans',sans-serif" }}>
                    Open and verify the identity document above before taking action. Approved users gain full platform access; rejected users are permanently removed.
                  </span>
                </div>
                <div style={{ display:"flex", gap:"10px" }}>
                  <ActionButton onClick={()=>approveUser(selectedUser._id)} label="Approve Account" variant="approve"/>
                  <ActionButton onClick={()=>rejectUser(selectedUser._id)}  label="Reject & Remove"  variant="reject"/>
                </div>
              </>
            ) : (
              <div style={{ display:"flex", alignItems:"center", gap:"10px", padding:"12px 16px", background:`${C.green}0a`, border:`1px solid ${C.green}22`, borderRadius:"8px" }}>
                <span style={{ fontSize:"16px" }}>✅</span>
                <div>
                  <div style={{ fontSize:"13px", fontWeight:"700", color:C.green }}>Account approved</div>
                  <div style={{ fontSize:"11px", color:C.muted, marginTop:"2px" }}>This user has already been verified and granted platform access.</div>
                </div>
              </div>
            )}
          </Modal>
              ...
              return (
                <div>
                  <div style={{ display:"grid", gridTemplateColumns:"auto 1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                    ...
                    <div style={{ background:C.s3, border:`1px solid ${C.border}`, borderRadius:"10px", padding:"14px", minHeight:"160px" }}>
